    ...
    for epoch in range(start_epoch, cfg.optim.epochs):
        ...
        for batch in data_loader:
            ...
            with accelerator.accumulate(model):
                ...
                # Forward pass
                teacher_global_cls_out, student_global_cls_out, student_local_cls_out = model(
                    global_crops=batch["global_views"], 
                    local_crops=batch["local_views"]
                )

                loss = criterion(
                    teacher_global_cls_out.chunk(2, dim=0),
                    student_global_cls_out.chunk(2 , dim=0) + \
                        student_local_cls_out.chunk(cfg.model.num_local_views, dim=0),
                    epoch=epoch,
                )
