...
def sanity_check_impressions(ct_data_impressions):
    ...
    Args:
        ct_data_impressions (iterable): Collection of impression strings.
...
def apply_with_checkpoint(df, source_column, target_column, func, checkpoint_interval, checkpoint_filename, extra_kwargs=None):
    ...
    If the target column already has a value (i.e., non-null), that row is skipped
    ...
    for idx, value in df[source_column].items():
        # Check if the target column already has a value; if so, reuse it.
        if pd.notnull(df.at[idx, target_column]):
            augmented = df.at[idx, target_column]
        else:
            # Skip processing if the source value is missing
            if pd.isnull(value):
                augmented = value
            else:
                if extra_kwargs:
